...
- Boolean-only schemas use the broadcast next-token path and report a constrained softmax over `true`/`false`.
- Enum schemas score each complete JSON-string candidate through its closing quote, then softmax the candidate joint log-probabilities. This is `candidate_joint_softmax`, **not calibrated probability**, and requires one full teacher-forced forward pass per distinct candidate token length.
- The historical benchmark table below predates this enum correction. Do not treat its one-pass/speed claims as evidence for multi-token enum scoring.
