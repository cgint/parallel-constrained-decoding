    ...
    def __init__(self, name: str, field_type: str, description: str, choices: Optional[List[str]] = None):
        ...
        if self.field_type == "boolean":
            ...
            if any(not isinstance(choice, str) or not choice.strip() for choice in choices):
                raise ValueError(f"Field '{name}' enum choices must be non-empty strings.")
            if len(set(choices)) != len(choices):
                raise ValueError(f"Field '{name}' enum choices must be unique.")
    ...
    def compile_parallel_metadata(self, tokenizer):
        ...
        for fname, fdef in field_items:
            if fdef.field_type == "boolean":
                ...
                # Enum values are scored as complete JSON-string continuations in
                # engine_mlx; first-token metadata is deliberately not used here.
                prefix = ""
                suffix = f'  "{fname}": "'
                cands = []
