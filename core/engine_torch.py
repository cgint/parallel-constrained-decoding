...
try:
    ...
    # Keep pure scoring helpers importable for deterministic unit tests on MLX-only hosts.
    torch = None
    F = None
    AutoModelForCausalLM = AutoTokenizer = TextIteratorStreamer = DynamicCache = None
...
def _require_torch_dependencies():
    if torch is None:
        raise RuntimeError(
            "Torch parallel generation requires the optional torch and transformers dependencies."
        ) from _TORCH_IMPORT_ERROR


def get_torch_engine():
    _require_torch_dependencies()
...
def _enum_candidate_sequences_torch(tokenizer, base_prompt: str, suffix: str, choices: List[str]):
    """Return complete JSON-string continuations after the already-open quote."""
...
def _score_enum_candidates_teacher_forced_torch(
    model, tokenizer, device, base_prompt: str, suffix: str, choices: List[str], temperature: float
) -> Tuple[List[float], List[float], int]:
    """Score complete enum JSON continuations with equal-length, no-cache batches."""
    _require_torch_dependencies()
    prefix_tokens, candidates = _enum_candidate_sequences_torch(tokenizer, base_prompt, suffix, choices)
    ...
    with torch.no_grad():
        for group in groups.values():
            inputs = torch.tensor([full_tokens for _, full_tokens, _ in group], dtype=torch.long, device=device)
            logits = model(inputs, use_cache=False).logits
            log_probs = F.log_softmax(logits, dim=-1)
            for row, (index, _full_tokens, continuation) in enumerate(group):
                score = sum(
                    float(log_probs[row, len(prefix_tokens) - 1 + offset, token_id].item())
                    for offset, token_id in enumerate(continuation)
                )
                raw_scores[index] = score

    scores_t = torch.tensor(raw_scores, dtype=torch.float64) / max(temperature, 1e-4)
    return raw_scores, F.softmax(scores_t, dim=-1).tolist(), len(groups)
...
def run_parallel_generation_torch(
    ...
    # 4. Boolean logits use the broadcast cache; enums are exact, separate
    # teacher-forced candidate continuations and never consume empty metadata.
    ...
    for i, (fname, fdef) in enumerate(field_items):
        if fdef.field_type == "boolean":
            ...
            all_probs = F.softmax(scores_t, dim=-1).tolist()
            ...
            suffix = f'  "{fname}": "'
            raw_scores, all_probs, enum_calls = _score_enum_candidates_teacher_forced_torch(
                model, tokenizer, device, base_prompt, suffix, fdef.choices, temperature
            )
            enum_forward_passes += enum_calls
            w_idx = max(range(len(all_probs)), key=all_probs.__getitem__)
            w_prob = float(all_probs[w_idx])
