...
def run_benchmark_suite(preset_paths: List[str], warmup: bool = True) -> List[Dict[str, Any]]:
    ...
    for path in preset_paths:
        ...
        work_kind = "Boolean broadcast (O(1))" if r.get("has_calibrated_probabilities") else "Enum candidate scoring"
        print(f"    Parallel Constrained    : {r['elapsed_ms']:>8.1f} ms | {work_kind:<24} | Passes: {r['sequential_forward_passes']}")
