...
> **Upstream Demo**: The original author's live demo on Hugging Face Spaces: [drinkmoonshine/parallel-constrained-decoding](https://huggingface.co/spaces/drinkmoonshine/parallel-constrained-decoding).
...
Parallel Constrained Decoding evaluates multi-field JSON schemas simultaneously rather than generating tokens sequentially. On an Apple Silicon M4 Max, it delivers **5.6x to 7.0x latency reductions** compared to standard autoregressive decoding with **100% schema validity** and **field-level confidence scores** (candidate joint softmax for enum fields; uncalibrated).

> **Known limitations:** On open classification presets (77+ enum choices), the engine's quality
> trails autoregressive LLMs by 2.5–3.5× (see `benchmarks/`). The leading explanation is
> gloss-asymmetry: the engine scores token-softmax over bare identifiers without semantic
> understanding of label meaning. Boolean fields and bounded-choice schemas (≤4 choices)
> retain the calibrated next-token path.
...
4. **Candidate Softmax Scores**: Normalized softmax scores are calculated over the candidate slice. Enum fields use `candidate_joint_softmax` (joint log-probability across the full candidate string, then softmax); Boolean fields use the calibrated next-token softmax over `true`/`false`.
...
```bash
git clone https://github.com/cgint/parallel-constrained-decoding.git
cd parallel-constrained-decoding

uv pip install -r requirements-mlx.txt
```

> **Note:** `requirements-mlx.txt` is the MLX (Apple Silicon) dependency set. The full `requirements.txt` includes the PyTorch stack for the Torch fallback backend.
...
```python
{
    "mode": "parallel_constrained_candidate_scored",
    "elapsed_ms": 74.5,
    "prefill_ms": 52.1,
    "suffix_eval_ms": 18.2,
    "sequential_forward_passes": 4,
    ...
    "field_telemetry": {
        "priority": {
            ...
            "cardinality": 4,
            "top_choices": [
                { "choice": "P0_CRITICAL", "probability": 0.9924, "joint_log_probability": -4.321 },
...
```bash
PYTHONPATH="$PWD" uv run python -m uvicorn server.app:app --host 127.0.0.1 --port 8000
```
...
```bash
PYTHONPATH="$PWD" uv run python -m core.benchmark
```
